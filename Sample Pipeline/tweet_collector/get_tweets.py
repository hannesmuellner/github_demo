...
import logging
import time
from config import cfg_dict, keyword_list, language_list
import pymongo

client = pymongo.MongoClient("mongodb://mongo_container:27017/")
db = client.tweets

consumer_key = cfg_dict['consumer_key']
consumer_secret = cfg_dict['consumer_secret']
access_token = cfg_dict['access_token']
access_token_secret = cfg_dict['access_token_secret']
...
def get_tweets(limit, callback):
    ...
    stream.filter(track=keyword_list, languages=language_list)
